...
Ray createCamRay(const int x_coord, const int y_coord, const int width, const int height, __constant Camera* cam, unsigned int* seed0, unsigned int* seed1){
...
float3 trace(__constant Sphere* spheres, const Ray* camray, const int sphere_count, const unsigned int* seed0, const unsigned int* seed1){

	Ray ray = *camray;

	float3 accum_color = (float3)(0.0f, 0.0f, 0.0f);
	float3 mask = (float3)(1.0f, 1.0f, 1.0f);
	unsigned int* randSeed0 = seed0;
	unsigned int* randSeed1 = seed1;
...
float3 convertColor(const float3* color, int samples)
{
    float r = color->x;
	float g = color->y;
	float b = color->z;
	...
	float random0, float random1, __global float3* accumbuffer, const int hashedframenumber)
{
	...
	for (unsigned int i = 0; i < SAMPLES; i++){
		...
		finalcolor += trace(spheres, &camray, sphere_count, &seed0, &seed1);
	}

    finalcolor = convertColor(&finalcolor, SAMPLES);
