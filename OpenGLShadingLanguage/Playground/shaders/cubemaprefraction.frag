...
void main() {
	...
	if (drawSkyBox) {
		...
		fragColor = mix(mix(vec4(finalColor, 1.0), reflectionColor, material.reflectionFactor), refractionColor, material.refractionFactor);
		// fragColor = vec4(vec3(material.reflectionFactor), 1.0);
	}
}